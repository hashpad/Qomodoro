...
Preferences::Preferences(QWidget *parent, Pomodoro *pm, Database *db)
    : QDialog(parent), ui(new Ui::Preferences), pm(pm), db(db) {
  ui->setupUi(this);
  ...
  ui->longBreakDurationLabel->setText(
      this->format_minutes(long_break_duration));
  ...
  ui->hideOtherNotification->setCheckState(
      (Qt::CheckState)hide_other_notifications);
}
...
void Preferences::on_startBreakFileChooser_clicked() {
  auto file_name = QFileDialog::getOpenFileName(
      this, tr("Open Audio File"), "/home/zebra",
      tr("Audio Files (*.mp3 *.wav *.ogg *.wma *.flac)"));
