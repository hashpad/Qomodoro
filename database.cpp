...
bool Database::setup() {
  ...
  qInfo() << "Setting up db";
  QSqlQuery query;
  query.prepare("CREATE TABLE pomodoros(ids integer primary key, duration "
                "integer, date DATE);");
  if (!query.exec())
    return false;

  query.prepare("CREATE TABLE breaks(ids integer primary key, duration "
                "integer, date DATE);");
  if (!query.exec())
    return false;
...
bool Database::add(int duration, QDate day, QString &type) {
  ...
  if (query.exec()) {
    if (query.next()) {
      ...
      query.prepare("UPDATE " + type +
                    " SET duration = (:duration) WHERE ids = (:id)");
      ...
      query.prepare("INSERT INTO " + type +
                    " (duration, date) VALUES (:duration, :day)");
...
bool Database::insert_break(int duration, QDate day) {
  QString type("breaks");
  return this->add(duration, day, type);
}
bool Database::insert_pm(int duration, QDate day) {
...
bool Database::has_pomodoros() {
  return m_db.tables().contains(QLatin1String("pomodoros"));
}
...
bool Database::prepare_config() {
  ...
  query.prepare("INSERT INTO config (" + into +
                ") VALUES (1500, 300, 600, 5, 0, 1, 0)");
  if (query.exec())
    return true;
  else
...
bool Database::set_break_duration(int duration) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET break_duration = (:duration) WHERE ids= 1");
      query.bindValue(":duration", duration);
      if (query.exec())
        return true;
      else
...
bool Database::set_long_break_duration(int duration) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET long_break_duration = (:duration) WHERE ids= 1");
      query.bindValue(":duration", duration);
      if (query.exec())
        return true;
      else
...
bool Database::set_cycles(int cycles) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET pomodoros_b4_long_break = (:cycles) WHERE ids= 1");
...
bool Database::set_start_break_sound(QString value) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET start_break_sound = (:value) WHERE ids= 1");
      query.bindValue(":value", value);
      if (query.exec())
        return true;
      else
...
bool Database::set_end_break_sound(QString value) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET end_break_sound = (:value) WHERE ids= 1");
      query.bindValue(":value", value);
      if (query.exec())
        return true;
      else
...
bool Database::set_hide_other_notifications(int value) {
  ...
  if (query.exec()) {
    if (query.first()) {
      query.prepare(
          "UPDATE config SET hide_other_notifications = (:value) WHERE ids= 1");
      query.bindValue(":value", value);
      if (query.exec())
        return true;
      else
