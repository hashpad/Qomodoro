...
QChart *Chart::get_day_view_chart() {
  ...
  *set0 << today_pomodoros << today_breaks;
  ...
  axisY->setRange(0, qMax(today_pomodoros + (10 - (today_pomodoros % 10)),
                          today_breaks + (10 - (today_breaks % 10))));
...
QChart *Chart::get_week_view_chart() {
  ...
  for (int day_i = 1; day_i <= 7; ++day_i) {
    *set0 << db->get_pomodoros(
                 QDate::currentDate().addDays(-day_of_week + day_i)) /
                 60;
    *set1 << db->get_breaks(
                 QDate::currentDate().addDays(-day_of_week + day_i)) /
                 60;
...
QChart *Chart::get_month_view_chart() {
  QDate from =
      QDate::currentDate()
          ...
          .addDays(1 - QDate::currentDate().addMonths(month_offset).day());

  QStringList categories;
  QBarSet *set0 = new QBarSet("Pomodoro");
  QBarSet *set1 = new QBarSet("Break");
  for (int i = 0; i < from.daysInMonth(); ++i) {
    *set0 << db->get_pomodoros(from.addDays(i)) / 60;
    *set1 << db->get_breaks(from.addDays(i)) / 60;
