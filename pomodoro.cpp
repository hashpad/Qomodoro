...
Pomodoro::Pomodoro(QObject *parent, Stopwatch *sw, int pm_duration,
                   int break_duration, int long_break_duration, int cycles,
                   Database *db)
    : QObject{parent}, sw(sw), pm_duration(pm_duration),
      break_duration(break_duration), long_break_duration(long_break_duration),
      cycles(cycles), db(db) {
...
bool Pomodoro::set_running() { return (this->running = !this->running); }
void Pomodoro::stop() {
  this->running = false;
...
void Pomodoro::reset_stopwatch() {
  if (this->state == PM)
    this->sw->set_value(this->pm_duration);
  if (this->state == SB)
    this->sw->set_value(this->break_duration);
  if (this->state == LB)
    this->sw->set_value(this->long_break_duration);
}
